...
def get_latest_version(current_version):
    ...
    url = "https://api.github.com/repos/rbxpi/rbxmanager/releases/latest"

    try:
        logger.debug(f"GET request sent to URL: {url}")
        response = requests.get(url)
        response.raise_for_status()  # Check if the request was successful
        data = response.json()
        latest_version = data.get("tag_name", "undefined")

        logger.debug(f"Latest version retrieved: {latest_version}")

    except requests.exceptions.RequestException as error:
        logger.error(f"HTTP request error: {error}")
        return None
...
def get_result_version(current_version, latest_version):
    ...
    logger.debug(f"Comparing current version ({current_version}) with the latest version ({latest_version})")
